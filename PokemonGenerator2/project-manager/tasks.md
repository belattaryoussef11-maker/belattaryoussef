...
- [x] Created `services/indexedDbService.ts` for managing `pokemons` and `settings` stores, including CRUD for Pokémon and token balance.
...
- [x] Show the Pokémon type after it is generated
- [x] Changer le prix de revente selon la rareté
- [x] Ajouter un système de trie basé sur la rareté
- [x] Ajouter un système de scoring au pokedex basé sur le nombre de pokemon possédés et revendus en valorisant d'avantage ceux possédés
- [x] Show attack and PV of pokemon
- [x] Show Pokémon attack with its name
